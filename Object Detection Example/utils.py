...
def get_bounding_boxes_for_prediction(prediction_tensor, batch_idx=0):
    ...
    bounding_boxes = cells_to_bboxes(first_split, anchors[0], consta nts.split_sizes[0]) + cells_to_bboxes(second_split, anchors[1], constants.split_sizes[1])
...
def get_label_boxes(labels, batch_index):
    ...
    return non_max_suppression(bounding_boxes,threshold=.6,iou_threshold=.5)
...
def mean_average_precision_and_recall(predictions, labels, iou_threshold, images):
    ...
    for index in range(len(labels[0])):
        ...
        if not (truePositives <= 0 and len(predicted_bounding_boxes)==0):
            precision = truePositives / (len(predicted_bounding_boxes))
            total_precision += precision
            precisions_counted += 1
            if falseNegative >0 or truePositives>0:
                recall = truePositives / (truePositives + falseNegative)
                total_recall += recall
                recall_counted += 1

    if precisions_counted == 0:
        return 0, 0
    if recall_counted == 0:
        return total_precision / precisions_counted , 0
